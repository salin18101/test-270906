 ...
 * @author Yanfly Engine Plugins + 改造
 *
 * @param ---General---
 * @default
 *
...
var _mu_sys_initialize = Game_System.prototype.initialize;
Game_System.prototype.initialize = function() {
	_mu_sys_initialize.call(this);
  this._muHシーン用ウィンドウ = false////メッセージウィンドウの幅や縁取りを変えるかの判定に使う
};

Window_Base.prototype.processNormalCharacter = function(textState) {
  var c = textState.text[textState.index++];
  var w = this.textWidth(c);
  if($gameSystem._muHシーン用ウィンドウ){
    this.contents.drawText(c, textState.x + 2 , textState.y, w * 2, textState.height);///太い縁取りと、メッセージウィンドウの一番左が干渉しないように文章を少し右にずらす
    textState.x += w + 1.5;////1文字ごとの行間を少し広げる
  }else{
    this.contents.drawText(c, textState.x + 1, textState.y, w * 2, textState.height);///太い縁取りと、メッセージウィンドウの一番左が干渉しないように文章を少し右にずらす
    textState.x += w;
  }
...
Window_Base.prototype.resetFontSettings = function() {
    ...
    if($gameSystem._muHシーン用ウィンドウ){
      this.contents.outlineColor = 'rgba(25, 25, 25, 0.9)';////縁取り文字の調整
      this.contents.outlineWidth = 5.5;// 6;//// 縁取り文字を太く$gameSystem.getMessageFontOutline() ;
    }else{
      this.contents.outlineColor = 'rgba(25, 25, 25, 0.8)';////縁取り文字の調整
