...
(function() {
  ...
  Game_Player.prototype.updateEventDrag = function() {
    ...
    if (this._dragEventId > 0) {
      $gameMap.event(this._dragEventId).requestBalloon(1);
     // $gameMap.event(this._dragEventId).setThrough(true);////独自処理。拾い上げる時にすり抜け状態に
  ...
  Game_Player.prototype.canDropLocate = function(dragEvent, x, y) {
    ...
    if ($gameMap.terrainTag(x, y) == 1) return false;////
