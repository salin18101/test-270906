...
////パラメーターから指定したキー以外にもctrlでも隠すようにした
////隠れている時から、決定キーや左クリックでも復帰できるようにした。そうしないとフリーズしたと勘違いする場合があるため。
///さらに改良。今はシフトで隠すようにはしていないがシフトキーがダッシュを兼ねているゲームだと、ダッシュして接触イベントのメッセージが表示された瞬間に非表示になりフリーズしたように感じてしまうので、メッセージウィンドウが開いていないときのシフトは反応しないようにした。
...
(function () {
    ...
    Window_Message.prototype.updateWait = function () {
        ...
        if (!this.isClosed() && this.isTriggeredHidden() || !this.isClosed() && Input.isTriggered("control")) {
            if (this.visible) {
                this.hide();
